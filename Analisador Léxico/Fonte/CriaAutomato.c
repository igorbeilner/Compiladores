...
/****************** Tokens *****************/
   ...
   G->adj[36][36].producao = 'M';		/* Token MAIN 					*/
   G->adj[37][37].producao = 'W';		/* Token WHILE 					*/
   G->adj[38][38].producao = 'I';		/* Token IF 					*/
   G->adj[39][39].producao = 'F';		/* Token END 					*/
   G->adj[40][40].producao = 'E';		/* Token ELSE 					*/
   G->adj[41][41].producao = 'T';		/* Token THEN					*/
   G->adj[42][42].producao = 'R';		/* Token RETURN 				*/
   G->adj[43][43].producao = 'V';		/* Token VAR 					*/
   G->adj[44][44].producao = 'B';		/* Token BREAK 					*/
   G->adj[46][46].producao = 'J';		/* Token OPERADORES				*/
   G->adj[48][48].producao = 'K';		/* Token CONSTANTES 			*/
   G->adj[49][49].producao = 'Z';		/* Token ESTADO DE ERRO 		*/

   G->adj[51][51].producao = 'A';		/* Token SOMA 			  |+|	*/
   G->adj[52][52].producao = 'S';		/* Token SUBTRACAO 		  |-|	*/
   G->adj[53][53].producao = 'D';		/* Token DIVISAO 		  |/|	*/
   G->adj[54][54].producao = 'X';		/* Token MULTIPLICACAO 	  |*|	*/
   G->adj[55][55].producao = 'L';		/* Token ABRE PARENTESES  |(|	*/
   G->adj[56][56].producao = 'P';		/* Token FECHA PARENTESES |)|	*/
   G->adj[57][57].producao = 'Q';		/* Token ATRIBUICAO 	  |=|	*/
   G->adj[58][58].producao = 'H';		/* Token NEGACAO		  |!|	*/
   G->adj[59][59].producao = 'N';		/* Token MENOR 			  |<|	*/
   G->adj[60][60].producao = 'C';		/* Token MAIOR 			  |>|	*/
   G->adj[61][61].producao = 'G';		/* Token PONTO E VIRGULA  |;|	*/
   G->adj[62][62].producao = 'Z';		/* Token ESTADO DE ERRO			*/
